...
const catalog_fetcher_1 = require("@rubic/catalog-fetcher");
const AUTH_JSON = "auth.json";
const TEMPLATE_JSON = "template.json";
function validate(obj, title, name) {
    let validator = catalog_fetcher_1.getValidator(name);
...
function update(fetchAll = false) {
    let f;
    let tmplFile = path.join(__dirname, "..", TEMPLATE_JSON);
    let destFile = path.join(__dirname, "..", catalog_fetcher_1.CATALOG_JSON);
    let root;
    let oldText;
    return Promise.resolve()
        ...
        if (fs.existsSync(authFile)) {
            options.auth = JSON.parse(fs.readFileSync(authFile, catalog_fetcher_1.JSON_ENCODING));
        }
        f = new catalog_fetcher_1.RubicCatalogFetcher(options);
        ...
        root = CJSON.parse(fs.readFileSync(path.join(__dirname, "..", TEMPLATE_JSON), catalog_fetcher_1.JSON_ENCODING));
        ...
        if (assigned > 0) {
            fs.writeFileSync(CJSON.stringify(root), catalog_fetcher_1.JSON_ENCODING);
        }
        ...
        // Load CATALOG_JSON
        f.logger.log(`Loading ${catalog_fetcher_1.CATALOG_JSON}`);
        oldText = fs.readFileSync(destFile, catalog_fetcher_1.JSON_ENCODING);
        let oldRoot = JSON.parse(oldText);
        try {
            // Varidate CATALOG_JSON
            validate(oldRoot, `${catalog_fetcher_1.CATALOG_JSON} (old)`);
        }
        catch (error) {
            // Ignore errors
            f.logger.warn(`Continue merging with invalid ${catalog_fetcher_1.CATALOG_JSON}`);
        }
        ...
        if (JSON.stringify(root) !== oldText) {
            ...
            validate(root, `${catalog_fetcher_1.CATALOG_JSON} (new)`);
            // Save
            f.logger.log(`Saving ${catalog_fetcher_1.CATALOG_JSON}`);
            fs.writeFileSync(destFile, JSON.stringify(root), catalog_fetcher_1.JSON_ENCODING);
        }
        else {
